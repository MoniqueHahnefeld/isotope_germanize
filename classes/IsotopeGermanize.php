...
class IsotopeGermanize extends FrontendTemplate
{
    ...
    public static function getTaxNotice()
    {
        ...
        $nonEUGuest = sprintf($GLOBALS['TL_LANG']['iso_legal']['notes']['nonEuGuest'], $strCountry);
        $nonEU = sprintf($GLOBALS['TL_LANG']['iso_legal']['notes']['nonEu'], $strCountry);
        $confirmedVatNo = sprintf($GLOBALS['TL_LANG']['iso_legal']['notes']['confirmedVatNo'], $objAddress->vat_no);
        $unconfirmedVatNo = sprintf($GLOBALS['TL_LANG']['iso_legal']['notes']['unconfirmedVatNo'], $objAddress->vat_no, $strCountry);
        $noVatNo = sprintf($GLOBALS['TL_LANG']['iso_legal']['notes']['noVatNo'], $strCountry);
...
//              echo '<pre>'.$strBuffer.'</pre>';
    ...
    protected function getPriceNotice($TaxClassLabel=false, $blnShippingExempt=false, $txt=false)
    {
        if ((FE_USER_LOGGED_IN === true && !self::isEuropeanUnion()) || (FE_USER_LOGGED_IN === true && !self::isGermany() && self::isEuropeanUnion() && self::hasValidVatNo())) {
            ...
            {
                $strNote = $GLOBALS['TL_LANG']['iso_legal']['priceNotes']['taxfree_shipping'];
                ...
                $strNote = $GLOBALS['TL_LANG']['iso_legal']['priceNotes']['taxfree_noShipping'];
            }
        } else {
            ...
            if (!$blnShippingExempt||$blnShippingExempt!=='true')
            {
                $strNote = $GLOBALS['TL_LANG']['iso_legal']['priceNotes']['tax_shipping'];
                ...
                $strNote = sprintf($GLOBALS['TL_LANG']['iso_legal']['priceNotes']['tax_noShipping'], $strTax);
